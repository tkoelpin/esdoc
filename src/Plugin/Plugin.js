...
class Plugin {
    ...
    #execHandler(handlerName, ev) {
        ...
        for (const item of this.#plugins) {
            const modulePath = item.name.match(/^[./]/u) ? item.name : `./node_modules/${item.name}`;
            const pluginPath = path.resolve(modulePath);

            const require = createRequire(path.resolve(`./node_modules`));
